...
class Aruba_upgrade():
	...
	def Pre_Post_check(self,check_type,log_file_path):
		try:
			...
			try:
				print(_warning+"\n===== Summary Report =====\n")
				# Verify all the config ID's
				cid = []
				cid_clr = _failed
				
				for summary in summary_data:
					_confid = summary.get("confid")
					cid.append(_confid)

				if len(set(cid)) > 1:
					# Miss-matched configuration ID
					cid_clr = _failed
				else:
					cid_clr = _success
				...
				for summary in summary_data:
					_t = summary.get("time")
					_type = summary.get("Type")
					_Host = summary.get("Host")
					_Partition = summary.get("Partition")
					_version = summary.get("version")
					_build = summary.get("build")
					_confid = summary.get("confid")
					_free_memory = summary.get("free_memory")
					_free_cpu = summary.get("free_cpu")
					_used_disk = summary.get("used_disk")

					try:
						_free_cpu = _free_cpu.split(" ")[1].split("%")[0].strip()
						if float(_free_cpu) < 20.0:
							_free_cpu = _failed + "Free-CPU:"+_free_cpu+"%"
						else:
							_free_cpu = _success + "Free-CPU:"+_free_cpu+"%"
					except:
						pass;

					try:
						_free_memory = _free_memory.split(" ")[1].split("%")[0].strip()
						if int(_free_memory) < 1000000:
							_free_memory = _failed + "Free-Memory:"+_free_memory+" Kb"
						else:
							_free_memory = _success + "Free-Memory:"+_free_memory+" Kb"
					except:
						pass;

					try: 
						
						_used_disk = _used_disk.split("%")
						_used_disk.remove("")
						dclr = _success
						for ud in _used_disk:
							if int(ud) > 79:
								dclr = _failed
						_used_disk = dclr + "Used Disks %"+ str(_used_disk) 
					
					except:
						pass;

					p = _warning+" (" + str(_type) + ") " + str(_Host)
					p = p + _info+" " +str(_Partition) 
					p = p + _info+" " + str(_version) + ":" + str(_build) + cid_clr +" Conf-ID:" + str(_confid)
					p = p + _info+" " + _free_cpu
					p = p + _info+" " + _free_memory
					p = p + _info+" " + _used_disk
						...
						if type(response) != dict:
							...
							if response != None:
								if response.get("_result").get("status") == 0:
									...
									print(_success+"=> AP Pre-load Executed:{}-{} => {}".format(host_name,host_ip,p))
									valid_state = True
									#self.logout(session,host_ip)
								else:
									p = response.get("_result").get("status_str")
