...
class Aruba_upgrade():
	...
	def upload_image_tftp_old(self,hostname,host_ip,img_file,disk,tftp_ip):
		...
		try:
			...
			if login_status[0] == True:
				...
				print("==> {}:{}- Copying file: {} from tftp:{}".format(hostname,host_ip,img_file,tftp_ip))
	...
	def upload_image_tftp(self,hostname,host_ip,img_file,disk,tftp_ip):
		...
		try:
			...
			if login_status[0] == True:
				...
				web_data = {'method': 'imtftp','args': 'tftp,'+tftp_ip+',zz,zz,'+img_file+','+partition.upper()+',unknown_host,none'}
			...
			if self.validate_image_upload(host_ip,disk,image_version,image_build) != True:
				...
				if self.yes_no("\n{}Image Version:{}-{} on Disk:{} Host:{}:{}  (Y/N)".format(msg,image_version,image_build,disk,hostname,host_ip)) == True:
					...
					if upload_type == "local":
						...
						self.upload_image_tftp(hostname,host_ip,img_file,disk,tftp_ip)
					else:
			...
			while last_skip == False and valid_state == False:
				try:
					...
					if res != None:
						...
						try:
							print("==>Validating AP Preload status")
	...
	def validate_image_upload(self,host_ip,disk,version,build):
		try:
			...
			if out != None:
				...
				if valid_image == True:
					print("=> New Image Installed = Host:{} Disk:{} Version:{} Build:{}"
						.format(host_ip,disk,version,build))
					return True
				else:
					print("=> Required image version for host:{} = Disk:{} Version:{} Build:{}"
						.format(host_ip,disk,version,build))
					return False
